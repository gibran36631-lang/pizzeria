...
export default function App() {
  ...
  const addToCart = (item, size = "full", qty = 1) => {
    const key = `${item.id}-${size}`;
    setCart((prev) => {
      const idx = prev.findIndex((i) => i.key === key);
      if (idx !== -1) { const next = [...prev]; next[idx] = { ...next[idx], qty: next[idx].qty + qty }; return next; }
      return [...prev, { key, id: item.id, name: item.name, size, qty, unitPrice: item.prices[size] }];
    });
  };
  ...
  const delivery = useMemo(() => (subtotal >= 400 || subtotal === 0 ? 0 : 30), [subtotal]);
  ...
  const ImageWithFallback = ({ src, alt, className }) => (<img src={src} alt={alt} className={className} loading="lazy" decoding="async" />);

  return (
    <div className="min-h-screen bg-[#faf4ea] text-stone-900">
      <header className="sticky top-0 z-50 backdrop-blur bg-[#faf4ea]/80 border-b border-stone-200/60">
        <div className="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between">
          <a href="#home" className="text-2xl tracking-[0.15em] font-serif">N I L U</a>
          <nav className="hidden md:flex gap-6 text-sm">
            <a href="#menu" className="hover:opacity-80">Menú</a>
            <a href="#promos" className="hover:opacity-80">Promociones</a>
            <a href="#historia" className="hover:opacity-80">Nuestra historia</a>
            <a href="#horarios" className="hover:opacity-80">Horarios</a>
            <a href="#ubicacion" className="hover:opacity-80">Ubicación</a>
            <a href="#ordenar" className="hover:opacity-80">Ordenar</a>
          </nav>
          <a href="#ordenar" className="px-4 py-2 rounded-full border border-stone-900 hover:-translate-y-[1px] transition inline-block text-sm">Ordenar</a>
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto px-5 pt-16 md:pt-24 pb-12 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <a href="#menu" className="px-5 py-3 rounded-full bg-stone-900 text-white hover:opacity-90">Ver menú</a>
              <a href="#ordenar" className="px-5 py-3 rounded-full border border-stone-900 hover:-translate-y-[1px] transition">Ordenar ahora</a>
            </div>
          </div>
              ...
              {MENU.map((m) => (
                <div key={\`ord-\${m.id}\`} className="bg-white border border-stone-200 rounded-2xl p-4">
                  <div className="flex gap-4">
                    <img src={m.img} alt={m.name} className="w-24 h-24 object-cover rounded-xl" />
            ...
            <form className="mt-5 space-y-3" onSubmit={handleCheckout}>
              ...
              <div className="pt-3 border-t">
                ...
                <div className="flex gap-3 mb-2 text-sm">
                  <label className="flex items-center gap-2"><input type="radio" name="pay" checked={order.payMethod === "card"} onChange={() => setOrder({ ...order, payMethod: "card" })} />Tarjeta</label>
                  <label className="flex items-center gap-2"><input type="radio" name="pay" checked={order.payMethod === "cash"} onChange={() => setOrder({ ...order, payMethod: "cash" })} />Efectivo (pagar al recibir)</label>
                </div>
              ...
              <button type="submit" className="w-full mt-2 px-4 py-2 rounded-full bg-stone-900 text-white text-sm disabled:opacity-60" disabled={cart.length === 0}>Pagar y confirmar pedido</button>
            </form>
          </aside>
        </div>
      </section>
